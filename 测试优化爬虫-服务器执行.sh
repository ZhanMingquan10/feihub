...
cat > /tmp/test_optimized_crawler.js << 'JSEOF'
...
echo "✅ 测试脚本已创建: /tmp/test_optimized_crawler.js"
echo ""
echo "=== 执行测试 ==="
cd /www/wwwroot/feihub/backend
node /tmp/test_optimized_crawler.js

echo ""
echo "=== 查看结果文件 ==="
echo "JSON 文件:"
ls -lh /tmp/feishu_extracted_*.json 2>/dev/null | tail -1
echo ""
echo "文本文件:"
ls -lh /tmp/feishu_extracted_*.txt 2>/dev/null | tail -1
echo ""
echo "查看文本内容（前500行）:"
cat /tmp/feishu_extracted_*.txt 2>/dev/null | tail -1 | head -500
