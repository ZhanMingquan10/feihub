...
export async function fetchFeishuDocument(link: string): Promise<FeishuDocumentData> {
  ...
  try {
    ...
    const extractedDate = await page.evaluate(() => {
      ...
      for (const selector of dateSelectors) {
        ...
        for (let i = 0; i < elements.length; i++) {
          ...
          // 优先查找 Modified/Created 开头的日期
          const modifiedMatch = text.match(/(?:Modified|Created)\s+(\w+)\s+(\d{1,2})(?:,\s*(\d{4}))?/i);
          if (modifiedMatch) {
            const month = modifiedMatch[1];
            const day = modifiedMatch[2].padStart(2, '0');
            const year = modifiedMatch[3] || currentYear;
          ...
          // 检查 "Month Day" 格式（如 November 30）
          const monthDayMatch = text.match(/\b(January|February|March|April|May|June|July|August|September|October|November|December)\s+(\d{1,2})(?:,\s*(\d{4}))?\b/i);
          if (monthDayMatch) {
    ...
    try {
      ...
      // 提取正文内容（排除标题和日期信息）
      content = await page.evaluate((titleText) => {
        // 尝试多种选择器获取正文内容
        const selectors = [
          '.wiki-content',
          '.doc-content',
        ...
        // 移除标题（如果有）
        if (titleText && fullText.startsWith(titleText)) {
          fullText = fullText.substring(titleText.length).trim();
        }

        // 移除 Modified/Created 日期行
        fullText = fullText.replace(/^\s*(?:Modified|Created)\s+\w+\s+\d{1,2}(?:,\s*\d{4})?\s*$/gim, '');

        // 移除其他可能的日期行
        fullText = fullText.replace(/^\s*(?:January|February|March|April|May|June|July|August|September|October|November|December)\s+\d{1,2}(?:,\s*\d{4})?\s*$/gim, '');
      ...
      try {
        // 获取渲染后的文本内容（不是HTML）
        const renderedText = await page.evaluate((titleText) => {
        ...
        const aiResult = await generateAIContentFromHTML(renderedText, dateHint);
