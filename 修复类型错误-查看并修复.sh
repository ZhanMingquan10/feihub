...
grep -n "content = await page.evaluate\|});" feishu-puppeteer.ts | head -10

echo ""
echo "=== 第二步：查看第414行到第500行的代码 ==="
sed -n '414,500p' feishu-puppeteer.ts | grep -n "});"
...
# 查找 content = await page.evaluate 的行号
evaluate_line = None
...
# 保存
with open('feishu-puppeteer.ts.bak7', 'w', encoding='utf-8') as f:
    f.writelines(lines)
